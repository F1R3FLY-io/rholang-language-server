...
#[allow(dead_code)]
pub struct LspMessageStream<R: BufRead + Read> {
    reader: R,
    message_buf: Vec<u8>,
...
#[allow(dead_code)]
impl<R: BufRead + Read> LspMessageStream<R> {
    ...
    fn next_byte(&mut self) -> Result<u8, String> {
        ...
        match self.reader.read(&mut bs) {
            Ok(0) => {
                Err("Input stream closed".to_string())
            }
            Ok(1) => {
                let b: u8 = bs[0];
            ...
            Ok(n) => {
                Err(format!("Expected no more than 1 byte to be read, but received: {}", n))
            }
            Err(e) => {
                Err(format!("Error reading byte: {}", e))
            }
        }
    }
            ...
            match b {
                b'\r' => {
                    ...
                    if line_length == 0 {
                        ...
                        return Err(
                            "Reached end of header section without defining the Content-Length".to_string()
                        );
                    }
                    return Err(format!(
                ...
                b':' => {
                    ...
                    let header_name = std::str::from_utf8(&self.message_buf[start..stop])
                        .map_err(|e| format!(
                            "Failed to convert header bytes to a UTF-8 string: {}", e
                        ))?;
    ...
    fn parse_body(&mut self) -> Result<String, String> {
        if let Some(content_length) = self.content_length {
            ...
            if let Err(e) = self.reader.read_exact(&mut self.message_buf[start..stop]) {
                return Err(format!("Error reading message body: {}", e));
            }
            let message = std::str::from_utf8(
                &self.message_buf[start..stop]
            ).map_err(|e| format!(
                "Failed to parse body from bytes: {:?}: {}",
                &self.message_buf[start..stop], e
            ))?.to_string();
            Ok(message)
        } else {
